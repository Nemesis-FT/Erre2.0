...
from erre2.database.crud import get_users, create_user, get_user, update_user
...
async def update_user_(user: schemas.UserCreatePlain, user_id: int, db: Session = Depends(get_db),
    ...
    h = bcrypt.hashpw(bytes(user.password, encoding="utf-8"), bcrypt.gensalt())
    user_n = update_user(db, schemas.UserCreate(name=user.name, surname=user.surname, hash=h, email=user.email), user_id)
    if user_n:
